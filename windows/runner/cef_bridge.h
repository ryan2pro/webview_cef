...
// Public contract of cef_bridge.dll.
//
// This header is shared by two very different callers:
//   * the runner executable (main.cpp / flutter_window.cpp), which uses it for
//     the one-time CEF process bootstrap on the process main thread, and
//   * Dart, which loads the same DLL through FFI and drives browsers at runtime.
//
// Because the runner exe is compiled with Flutter's apply_standard_settings()
// (/W4 /WX), this header must stay free of any CEF include. All types crossing
// the boundary are plain C / Win32 handles.
//
// Threading contract:
//   * cef_bridge_execute_process / cef_bridge_initialize / cef_bridge_shutdown
...
/// Bootstraps CEF for the current process.
///
...
///
/// Must be the very first CEF call in the process, before any Flutter setup.
...
/// main thread, after cef_bridge_execute_process() reported -1.
///
/// \param cache_root_path Absolute UTF-8 path used as CefSettings.root_cache_path.
///        Intermediate directories are created when missing.
...
/// process main thread after the application message loop has exited. Waits up
/// to a few seconds for browsers to finish closing, then returns.
